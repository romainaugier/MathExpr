...
bool SSA::build_from_ast(const AST& ast) noexcept
{
    ...
    std::unordered_map<const ASTNode*, SSAStmt*> mapping;
    ...
    auto traverse = [&](auto&& self, const ASTNode* node) {
        ...
        switch(node->type_id())
        {
            case ASTNodeTypeId_Variable:
            {
                ...
                SSAStmt* variable = this->_slab.allocate<SSAStmtVariable>(variable_node->get_name(),
                                                                          version++);
            ...
            case ASTNodeTypeId_Literal:
            {
                ...
                SSAStmt* literal = this->_slab.allocate<SSAStmtLiteral>(literal_node->get_name(),
                                                                        version++);
            ...
            case ASTNodeTypeId_UnOp:
            {
                ...
                SSAStmt* unop = this->_slab.allocate<SSAStmtUnOp>(mapping[unop_node->get_operand()],
                                                                  unop_node->get_op(),
                                                                  version++);
            ...
            case ASTNodeTypeId_BinOp:
            {
                ...
                SSAStmt* binop = this->_slab.allocate<SSAStmtBinOp>(mapping[binop_node->get_left()],
            ...
            case ASTNodeTypeId_FuncOp:
            {
                ...
                for(const auto& argument : funccall_node->get_arguments())
                {
                    ...
                    arguments.push_back(mapping[argument]);
                }

                SSAStmt* funccall = this->_slab.allocate<SSAStmtFunctionOp>(funccall_node->get_function_name(),
                                                                            std::move(arguments),
                                                                            version++);
